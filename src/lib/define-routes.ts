...
function buildPathFromPattern(
  ...
  return pattern.replace(/:(\w+)/g, (_, k: string) => {
    const v = params[k];
    if (v == undefined) {
...
export function buildPath<
  ...
  const N extends keyof Patterns & string,
>(
  name: N,
  patterns: Patterns,
...
export function defineRoutes<const Patterns extends Record<string, string>>(
  patterns: Patterns
) {
  ...
  const boxes = {} as { [K in Names]: RouteBox<K, Patterns[K] & string> } & {
    toPath: <N extends Names>(
      name: N,
      params?: ParamsOf<Patterns[N] & string>
    ) => string;
    matchPath: (
      path: string
    ) => RouteMatch<Names, Patterns[Names] & string> | null;
    matchAllPaths: (
      path: string
    ) => RouteMatch<Names, Patterns[Names] & string>[];
  ...
  for (const name of Object.keys(patterns) as Names[]) {
    ...
    const box: RouteBox<any, any> = {
      ...
      to: (params?: Record<string, string>) =>
        buildPathFromPattern(pattern, params),
      match: (path: string) => {
        ...
        for (const [i, k] of compiled.keys.entries()) {
  ...
  (boxes as any).toPath = (name: Names, params?: any) =>
    buildPath(name, patterns, params as any);
  // Legacy alias
  (boxes as any).to = (name: Names, params?: any) =>
    (boxes as any).toPath(name, params);
  ...
  (boxes as any).matchAllPaths = (path: string) => {
    const entries = (Object.keys(patterns) as Names[])
      ...
      .sort(
        (a, b) =>
          specificityScore(a.compiled.pattern) -
          specificityScore(b.compiled.pattern)
      )
      .reverse();
    ...
    for (const e of entries) {
      ...
      const params: Record<string, string> = {};
      for (const [i, k] of e.compiled.keys.entries()) {
        params[k] = decodeURIComponent(m[i + 1]);
      }
  ...
  (boxes as any).matchAll = (path: string) =>
    (boxes as any).matchAllPaths(path);
