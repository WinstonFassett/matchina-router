...
export function createRouter<const Patterns extends Record<string, string>>(
  ...
  type ParamsOf<N extends RouteName> = Parameters<
    Defs[N]["to"]
  >[0] extends undefined
    ? {}
  ...
  type Ctx = {
    ...
    navDir: Direction; // computed per navigation, stable for this render
  };
  ...
  type RouteProps = {
    [K in RouteName]:
      ...
      | ({ name: K } & {
          view: React.ComponentType<ParamsOf<K>>;
          children?: React.ReactNode;
          index?: boolean;
          viewer?: React.FC<any>;
          keep?: number;
  ...
  const RouterProvider: React.FC<{ children?: React.ReactNode }> = ({
    ...
    const to = path
      ? (defs.matchPath(path) as RouteMatch<RouteName, any> | null)
      : null;
    // Prefer previous path from the store change (handles rapid multi-push correctly)
    const changePrevPath =
      (change as any)?.fromPath ||
      (change as any)?.from?.path ||
      (change as any)?.prevPath ||
      (change as any)?.previousPath ||
      "";
    const effectivePrevPath = changePrevPath || prevPath;
    const from = effectivePrevPath
      ? (defs.matchPath(effectivePrevPath) as RouteMatch<RouteName, any> | null)
      : null;
    ...
    const currIdx = (() => {
      ...
      return typeof st.__vtIdx === "number"
        ? (st.__vtIdx as number)
        : prevIdxRef.current;
    })();
  ...
  const RouterSnapshotProvider: React.FC<{
    value: Ctx;
    children?: React.ReactNode;
  }> = ({ value, children }) => {
    return (
  ...
  function useNavigation() {
    ...
    const toPath = <N extends RouteName>(name: N, params?: ParamsOf<N>) =>
      (defs as any).toPath(name, params);
    const goto =
      <N extends RouteName>(name: N, params?: ParamsOf<N>) =>
      () =>
        history.push(toPath(name, params));
    const replace =
      <N extends RouteName>(name: N, params?: ParamsOf<N>) =>
      () =>
        history.replace(toPath(name, params));
  ...
  const Routes: React.FC<{
    ...
    const { to, from, navDir } = ctxAll as any;
    if (!viewer) return null;
    const direction: Direction = navDir;
    ...
    const autoChild =
      inScope && to && views
        ? React.createElement(
            views[to.name as any] as React.ComponentType<any>,
            { ...(to as any).params }
          )
        : null;
  ...
  type LinkProps = {
    [K in RouteName]: { name: K; params?: ParamsOf<K> };
  }[RouteName] & {
    children?: React.ReactNode;
  ...
  const Link: React.FC<LinkProps> = ({ name, params, children, ...a }) => {
    ...
    try {
      path = (defs as any).toPath
        ? (defs as any).toPath(name, params)
        : (defs as any).buildPath(name, params);
    } catch (e) {
      path = null;
    }
