...
import ViewSlider, { type SimpleViewSliderProps } from 'react-view-slider/simple';
...
export const DrilldownViewer: React.FC<OurDrilldownProps> = (props) => {
  ...
  if (typeof viewerRender === 'function') {
    ...
    const depthKey = typeof getDepth === 'function' ? getDepth(viewKey) : undefined;
    childKey = depthKey != null ? String(depthKey) : (viewKey != null ? String(viewKey) : '0');
    ...
    const routes: Array<{ el: React.ReactElement<DDRouteProps>; index: number }> = [];
    React.Children.forEach(children, (el) => {
      if (!React.isValidElement(el)) return;
      if (el.type === DDRoute) routes.push({ el: el as React.ReactElement<DDRouteProps>, index: routes.length });
    });

    // Pick the first matching route by name and optional predicate
    let selected: { el: React.ReactElement<DDRouteProps>; index: number } | null = null;
    for (const entry of routes) {
      const p = entry.el.props as DDRouteProps;
      if (p.name === routeName && (!p.when || p.when({ name: routeName!, params }))) {
        selected = entry;
        break;
      }
    }
    ...
    const Comp = (selected.el.props as DDRouteProps).component as React.ComponentType<any>;
    childNode = <Comp {...params} />;
    childKey = selected.el.key != null ? String(selected.el.key) : String(selected.index);
  }
